...
import pickle
import csv
# Replace coverage and quality functions with log-likelihood and KL divergence
from weight_learning.weight_evaluators import compute_kl_divergence

def compute_log_likelihood(model, test_data):
    """Compute average log-likelihood on test data."""
    model.eval()
    with torch.no_grad():
        result = model.forward(test_data)
        return result['mixture_log_prob'].mean().item()

def compute_kl_divergence_metric(target_samples, generated_samples):
    """Compute KL divergence between target and generated samples."""
    target_np = target_samples.detach().cpu().numpy()
    generated_np = generated_samples.detach().cpu().numpy()
    return compute_kl_divergence(target_np, generated_np)
...
def evaluate_single_sequential_dataset(dataset_name):
    ...
    # Compute new metrics instead of coverage/quality
    log_likelihood = compute_log_likelihood(model, test_data)
    kl_divergence = compute_kl_divergence_metric(test_data, generated_samples)
    diversity_metrics = evaluate_flow_diversity(model)
    
    # For Sequential model, use the forward method to get log probability
    with torch.no_grad():
        result = model.forward(test_data)
...
def comprehensive_sequential_evaluation():
    ...
    print(f"{'='*80}")
    print(f"{'Dataset':<15} | {'Coverage':<8} | {'Quality':<8} | {'Log Prob':<10} | {'Flow Sep':<8} | {'Weights Trained':<14}")
    print("-" * 95)
    ...
    for dataset_name, results in all_results.items():
        ...
        print(f"{dataset_name:<15} | {results['coverage']:<8.3f} | {results['quality']:<8.3f} | "
              f"{results['log_probability']:<10.3f} | {results['flow_separation']:<8.3f} | {weights_status:<14}")
    ...
    with open(os.path.join(results_dir, 'sequential_comprehensive_metrics.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['dataset', 'coverage', 'quality', 'log_probability', 'flow_separation', 'weights_trained'])
    ...
    for dataset_name, results in all_results.items():
        ...
        print(f"{'Flow':<8} | {'Coverage':<8} | {'Quality':<8} | {'Log Prob':<10} | {'Weight':<8}")
        print("-" * 60)
        ...
        if 'individual_flow_metrics' in results:
            for i, (flow_name, metrics) in enumerate(results['individual_flow_metrics'].items()):
                ...
                print(f"{flow_name.upper():<8} | {metrics['coverage']:<8.3f} | {metrics['quality']:<8.3f} | "
                      f"{metrics['log_probability']:<10.3f} | {weight:<8.3f}")
    ...
    with open(os.path.join(results_dir, 'sequential_individual_flow_metrics.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['dataset', 'flow_name', 'coverage', 'quality', 'log_probability', 'learned_weight'])
    ...
    print(f"\n🏆 Best performing Sequential dataset (mixture): {best_dataset[0]} (Coverage: {best_dataset[1]['coverage']:.3f})")

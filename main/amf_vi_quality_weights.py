...
class SequentialAMFVI(nn.Module):
    """Sequential training version of AMF-VI with quality-based weights."""
    ...
    def compute_flow_quality_weights(self, n_samples=500):
        """Compute quality-based weights for each flow."""
        ...
        # Convert to tensor and apply softmax for normalization
        flow_qualities = torch.stack(flow_qualities)
        weights = torch.softmax(flow_qualities, dim=0)
    ...
    def forward(self, x):
        """Forward pass with quality-based or uniform weights."""
        if not self.flows_trained:
    ...
    def sample(self, n_samples):
        """Sample from the mixture with quality-based or uniform weights."""
        device = next(self.parameters()).device
        
        if self.use_quality_weights:
            # Get quality-based weights
            flow_weights = self.compute_flow_quality_weights()
...
def train_sequential_amf_vi(dataset_name='multimodal', show_plots=True, save_plots=False, use_quality_weights=True):
    """Train sequential AMF-VI with quality-based or uniform weights."""
    ...
    model = SequentialAMFVI(dim=2, flow_types=['realnvp', 'maf', 'iaf'], use_quality_weights=use_quality_weights)
    ...
    with torch.no_grad():
        ...
        if use_quality_weights:
            ...
            print(f"\n🎯 Flow Quality Weights: {flow_weights.cpu().numpy()}")
        ...
        title_suffix = " (Quality Weights)" if use_quality_weights else " (Uniform Weights)"
        ...
        plt.tight_layout()
        plt.suptitle(f'Sequential AMF-VI Results ({weight_type} Weights) - {dataset_name.title()}', fontsize=16)
    ...
    with open(model_path, 'wb') as f:
        pickle.dump({'model': model, 'losses': flow_losses, 'dataset': dataset_name, 'use_quality_weights': use_quality_weights}, f)
...
if __name__ == "__main__":
    ...
    for dataset_name in datasets:
        ...
        model_quality, flow_losses_quality = train_sequential_amf_vi(
            dataset_name, 
            show_plots=False, 
            save_plots=True,

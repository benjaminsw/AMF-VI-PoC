...
def evaluate_single_dataset(dataset_name):
    ...
    # Load pre-trained adaptive model (UPDATED PATH)
    results_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'results')
    model_path = os.path.join(results_dir, f'trained_model_adaptive_{dataset_name}.pkl')
    
    if not os.path.exists(model_path):
        print(f"❌ Adaptive model file not found at {model_path}")
        print("Please run the adaptive training script first: python sequential_amf_vi_adaptive.py")
        return None
    ...
    # Flow names to match adaptive version
    ...
    # NEW: Evaluate adaptive sampling behavior
    ...
    results = {
        ...
        'adaptive_sampling_metrics': adaptive_sampling_metrics  # NEW
    }
    ...
    # NEW: Print adaptive sampling analysis
...
def comprehensive_evaluation():
    ...
    for i, (dataset_name, results) in enumerate(all_results.items()):
        ...
        # Row 3: Adaptive vs Uniform comparison (NEW)
    ...
    # NEW: Create adaptive sampling comparison CSV
    ...
    # NEW: Dataset with most effective adaptive sampling
    if adaptive_comparison_data:

...
def evaluate_single_sequential_dataset(dataset_name):
    ...
    # UPDATED: Dynamically determine flow names based on actual flows in the model
    flow_type_map = {
        'RealNVPFlow': 'realnvp',
        'MAFFlow': 'maf', 
        ...
        'GlowFlow': 'glow',
        'TANFlow': 'tan',
        'RBIGFlow': 'rbig'  # ADDED: Support for RBIG
    }
    ...
    # UPDATED: Compute percentage improvements for all flows dynamically
    ...
    results = {
        ...
        'kl_divergence': kl_divergence,
        'cross_entropy_surrogate': cross_entropy,
        'percentage_improvements': percentage_improvements,  # UPDATED: Store all improvements
        ...
        'flow_names': flow_names  # UPDATED: Store actual flow names
    }
...
def comprehensive_sequential_evaluation():
    ...
    # UPDATED: Extended colors for more flows
    colors = ['steelblue', 'crimson', 'forestgreen', 'darkorange', 'purple', 'brown', 'pink', 'gray', 'olive', 'cyan']
    ...
    for i, (dataset_name, results) in enumerate(all_results.items()):
        ...
        if results['weights_trained']:
            # UPDATED: Plot learned weights dynamically
            weights = results['learned_weights']
            flow_names = [name.upper() for name in results['flow_names']]
            bar_colors = ['green', 'orange', 'purple', 'brown', 'pink', 'gray', 'olive', 'cyan'][:len(weights)]
            bars = axes[2, i].bar(flow_names, weights, color=bar_colors)
    ...
    # UPDATED: Create summary table with dynamic flow comparisons
    print(f"\n{'='*80}")
    ...
    for dataset_name, results in all_results.items():
        ...
        if 'individual_flow_metrics' in results:
            for i, (flow_name, metrics) in enumerate(results['individual_flow_metrics'].items()):
                weight = results['learned_weights'][i] if results['weights_trained'] else 1/len(results['learned_weights'])
    ...
    # UPDATED: Best performing dataset based on percentage improvements (dynamic)
    for flow_name in all_results[list(all_results.keys())[0]]['flow_names']:
        improvement_key = f'vs_{flow_name}'
        improvement_data = [(name, results['percentage_improvements'].get(improvement_key, 0)) for name, results in all_results.items()]
        best_improvement = max(improvement_data, key=lambda x: x[1])
        print(f"🏆 Best performing Sequential dataset (% improvement vs {flow_name.upper()}): {best_improvement[0]} (Improvement: {best_improvement[1]:.1f}%)")
...
if __name__ == "__main__":
    # UPDATED: No need to monkey patch - the evaluation will work with whatever flows are in the saved models

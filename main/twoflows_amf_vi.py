...
from weight_learning import WeightLearnerFactory, WeightStrategyFactory, UpdateSchedulerFactory, WeightEvaluator
...
class SequentialAMFVI(nn.Module):
    ...
    def __init__(self, dim=2, flow_types=None, weight_update_method='log_likelihood'):
        ...
        # NEW: Weight learning components
        if weight_config is not None:
            self.weight_config = weight_config
            self.weight_learner = WeightLearnerFactory.create_learner(weight_config)
            self.weight_strategy = WeightStrategyFactory.create_strategy(weight_config)
            self.update_scheduler = UpdateSchedulerFactory.create_scheduler(weight_config)
            self.weight_evaluator = WeightEvaluator(weight_config)
        else:
            self.weight_config = None
    
    def train_mixture_weights_modular(self, data):
        """Stage 2: Modular weight learning."""
        if self.weight_config is None:
            return self.train_mixture_weights(data)  # Fallback to original
        
        print("🔄 Stage 2: Learning mixture weights (modular)...")
        weight_losses = self.weight_learner.train_weights(
            self, data, self.weight_strategy, self.update_scheduler
        )
...
def train_sequential_amf_vi(dataset_name='multimodal', show_plots=True, save_plots=False):
    ...
    weight_config = None
    if config_path:
        weight_config = load_config(config_path)
    ...
    model = SequentialAMFVI(dim=2, flow_types=['realnvp', 'maf'], weight_config=weight_config)
    model = model.to(device)
    
    # Stage 1: Train flows independently
    flow_losses = model.train_flows_independently(data, epochs=200, lr=1e-3)
    
    # Stage 2: Learn mixture weights (modular or original)
    if weight_config:
        weight_losses = model.train_mixture_weights_modular(data)
    else:
...
if __name__ == "__main__":
    ...
    # Configuration path
    config_path = "config/weight_config.yaml"
    ...
    for dataset_name in datasets:
        ...
        model, flow_losses, weight_losses = train_sequential_amf_vi_modular(
